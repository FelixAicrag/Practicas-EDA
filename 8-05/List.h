...
public:
	...
	*/
	...
	//EJERCICIO 8-05 A la cola PepsiCola
	void jump(const T pringao, List <T>colegas){
		List<T>::Iterator it = begin();

		while (it != end() && it.elem() != pringao) { it.next(); }; //Pongo el iterador en la posicion correcta.

		if (it != end()) { //Si hemos encontrado el centinela.
			List<T>::Iterator it2 = it;
			it2.next();
			List <T> misColegas = colegas; //Llamo al constructor por copia.

			if (it == _ult) _ult = misColegas._ult; //Caso particular: La lista se anyade justamente al final.

			List<T>::Iterator itColegas = misColegas.begin(); //Iterador al principio de los colegas.
			it._act->_sig = misColegas._prim; //El siguiente elemento es el primero de la lista.


			while (itColegas._act->_sig != NULL) { 
				//Avanzamos hasta el ultimo elemento ocupado.
				itColegas.next();
			}

			itColegas._act->_sig = it2._act; //El ultimo de los colegas enlaza con el nodo que antes iba despues del centinela.
			
			misColegas._prim = NULL;
			misColegas._ult = NULL;
		}
	}
